...
export default function AddTopping() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3000/addToppings', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        setMessage('Error adding topping.');
      }
    } catch (error) {
      console.error('Error:', error);
      setMessage('An error occurred while adding the topping.');
    }
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <button type="submit" style={styles.button}>Create Topping</button>
      </form>
    </div>
  );
}
